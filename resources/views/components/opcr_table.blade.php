...
<div class="d-flex justify-content-between">
    ...
    <button type="button" class="btn btn-primary my-2" data-file-name="{{$printProvince}}_OPCR-{{$opcrs_active[0]->opcr_ID}}_{{$opcrs_active[0]->year}}" id="print-scoreCard">Scorecard</button>
    </div>
        ...
        @foreach ($objectivesact as $objective)
            @if (count($objective->measures) > 0)
                ...
                @php
                    ...
                    $measures = $objective->measures()->orderBy('number_measure', 'asc')->get();
                @endphp
                    ...
                    @if ($measure->type == 'DIRECT' || $measure->type == 'DIRECT MAIN')
                        <tr>
                            ...
                            @if (isset($annual_targets[$measure->strategic_measure_ID][$user->province_ID]))
                                ...
                                <td>{{number_format((($monthly_targets2[$measure->strategic_measure_ID]->total_accom/$monthly_targets2[$measure->strategic_measure_ID]->total_targets)*100), 2)}}%</td>
                                ...
                                <td>{{number_format((($monthly_targets2[$measure->strategic_measure_ID]->first_sem_accom/$monthly_targets2[$measure->strategic_measure_ID]->first_sem)*100), 2)}}%</td>
                                ...
                                <td>{{number_format((($monthly_targets2[$measure->strategic_measure_ID]->second_sem_accom/$monthly_targets2[$measure->strategic_measure_ID]->second_sem)*100), 2)}}%</td>
